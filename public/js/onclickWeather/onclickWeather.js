$(document).ready(_ => {
  ...
  $("#submitCityName").on("click", () => {
    let requestedLocation = $("#weatherLocation")
      .val()
      .trim();
    let queryURL =
    ...
    $.ajax({
      ...
      .then(data => {
        ...
        function sunsetBackground() {
          $("#body").css({
            "background-image": "linear-gradient(#5F0066, #FF0500)"
          });
        }
        ...
        let highTempDiv = $(
          `<div>High: ${highF}&#176 F / ${highC}&#176 C</div>`
        ).addClass("extra-temps");
        ...
        let lowTempDiv = $(
          `<div>Low: ${lowF}&#176 F / ${lowC}&#176 C</div>`
        ).addClass("extra-temps");
        ...
        let sunrise = $(`<div>Sunrise: ${sunriseTime}</div>`).addClass(
          "sunrise"
        );
        ...
        let supplementalInfo = $("<div>")
          .addClass("supplemental-current-info")
          .append(description)
          .append(timeDiv)
          .append(visibility)
          .append(pressure)
        ...
        let currentConditionsDiv = $(
          `<div class='current-conditions'>Current Conditions: <br> ${currentWeatherConditions}</div>`
        );

        let currentWeatherInfo = $("<div>")
          .addClass("all-current-info")
          .append(currentConditionsDiv)
          .append(allTemps);

        let displayIcon = data.weather[0].icon;
        let weatherIconURL =
          "https://openweathermap.org/img/w/" + displayIcon + ".png";
        let weatherIcon = $("<img alt='img'>")
          .attr("src", weatherIconURL)
          .addClass("weather-icon");

        $("#currentWeather")
          .append(currentWeatherInfo)
          .append(weatherIcon)
